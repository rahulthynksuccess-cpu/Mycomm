// FULL UPDATED FILE GENERATED FROM YOUR ACTUAL PROJECT
...
const safeFallback =
fallback && !fallback.includes('@') && !fallback.includes(':')
? fallback.trim()
: null;
...
console.log(`[WA] Client already exists for ${accountId}`);
...
console.log(`[WA] Using file auth for ${accountId}`);
...
if (connection === 'open') {
  ...
  [2000, 5000, 10000, 20000, 30000].forEach(delay => {
    setTimeout(() => {
      pushChats();
    }, delay);
  });
  ...
  setTimeout(async () => {
    try {
      const chats = await sock.fetchAllParticipating();
      ...
      console.log('[WA] Full sync failed:', e.message);
    }
  }, 7000);
}
...
if (connection === 'close') {
  ...
  if (loggedOut || badSession) {
    ...
    setTimeout(() => {
      if (!clients[accountId]) {
        createClient(accountId, io).catch(console.error);
      }
    }, 8000);
  }
}
...
```
  return fs.existsSync(
    path.join(SESSIONS_DIR, d, 'creds.json')
  );
})
