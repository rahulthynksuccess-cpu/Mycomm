...
export default function EmailTab() {
  ...
  const loadEmails = useCallback(async () => {
    ...
    try {
      const result = await emailAPI.getMessages(activeAccount, { folder: activeFolder, page, limit: 50 });
  ...
  async function deleteSelected() {
    ...
    for (const uid of selectedEmails) {
      await emailAPI.delete(activeAccount, uid, activeFolder).catch(() => {});
    }
  ...
  async function flagEmail(uid, flagName, add) {
    try {
      ...
      setEmails(prev => prev.map(e => e.uid === uid
        ? { ...e, flags: add ? [...e.flags, flagName] : e.flags.filter(f => f !== flagName) }
        : e
      ));
    } catch (e) { console.error(e); }
  }
  ...
  async function doSearch(q) {
    ...
    setLoadingList(true);
    try {
      const results = await emailAPI.search(activeAccount, q, activeFolder);
  ...
  const displayList = searchResults !== null ? searchResults : emails;
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                    <span className="item-sender" style={{ fontSize: 13, color: 'var(--text)', fontWeight: !email.isRead ? 600 : 400, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 150 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text3)' }}>
                      {parseEmail(activeEmail.from)} → {activeEmail.to}
                    </div>
        ...
        <div className="modal-overlay" onClick={() => setComposing(false)}>
          <div className="modal" style={{ width: 600 }} onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body" style={{ gap: 10 }}>
              ...
              <div className="field">
                <label>To *</label>
                <input className="input" placeholder="recipient@email.com" value={compose.to} onChange={e => setCompose({ ...compose, to: e.target.value })} />
              </div>
              <div className="field">
                <label>CC</label>
                <input className="input" placeholder="cc@email.com" value={compose.cc} onChange={e => setCompose({ ...compose, cc: e.target.value })} />
              </div>
              <div className="field">
                <label>Subject *</label>
              ...
              <div className="field">
                <label>Message</label>
                <textarea className="input" rows={10} placeholder="Write your email…" value={compose.body} onChange={e => setCompose({ ...compose, body: e.target.value })} />
              </div>
            </div>
...
const folderIcon = f => ({ INBOX: '📥', Sent: '📤', Drafts: '📝', Spam: '🚫', Trash: '🗑️' }[f] || '📁');
const parseName = (from = '') => { const m = from.match(/^"?([^"<]+)"?\s*</); return m ? m[1].trim() : from.split('@')[0]; };
const parseEmail = (from = '') => { const m = from.match(/<([^>]+)>/); return m ? m[1] : from; };
const initials = (n = '') => n.split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase() || '?';
const strColor = (s = '') => {
  const colors = ['#5b4fcf','#2d6a4f','#7b2d8b','#1a5276','#784212','#6d4c41'];
